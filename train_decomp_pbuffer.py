...
def validation(models, dataloader, eps, criterion, device, epoch, use_llpm_buf, mode='kpcn'):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, leave=False, ncols=70):
            ...
            mask, batch1, batch2, p_buffer, noisy = decompNet(batch)
...
def train(mode, 
    ...
    print(mode)
    decompNet = decompModule(in_channel=32, out_channel=1, discrete=do_discrete, use_pbuffer=True).to(device)
    ...
    for epoch in range(init_epoch, epochs):
        ...
        for batch in tqdm(dataloader, leave=False, ncols=70):
            ...
            loss_manif_decomp = 0.1 * path_criterion(p_buffer, noisy)
            loss_manif = None
            paths = batch2['paths'].to(device)
            p_buffer_diffuse, p_buffer_specular = diffPathNet(paths), specPathNet(paths)
            '''Feature Disentanglement'''    
