...
def train(mode, 
    ...
    # checkpointDecomp = torch.load('trained_model/kpcn_decomp_mask/decomp_e2.pt')
    # decompNet.load_state_dict(checkpointDecomp['model_state_dict'])
    # optimizerDecomp.load_state_dict(checkpointDecomp['optimizer_state_dict'])
    ...
    preDiffCkpt = torch.load('trained_model/kpcn/diff_e10.pt')
    preSpecCkpt = torch.load('trained_model/kpcn/spec_e10.pt')
    preDiffuseNet.load_state_dict(preDiffCkpt['model_state_dict'])
    preSpecularNet.load_state_dict(preSpecCkpt['model_state_dict'])
    preDiffuseNet.eval()
    preSpecularNet.eval()
    ...
    # ckptDecomp = torch.load('trained_model/kpcn_decomp_mask_2/decomp_e12.pt')
    # decompNet.load_state_dict(ckptDecomp['model_state_dict'])
    # optimizerDecomp.load_state_dict(ckptDecomp['optimizer_state_dict'])
    decompNet.train()
    
    # checkpointDiffPath = torch.load('trained_model/kpcn_decomp_mask_2/path_diff_e12.pt')
    ...
    # checkpointSpecPath = torch.load('trained_model/kpcn_decomp_mask_2/path_spec_e12.pt')
    ...
    # checkpointDiff1 = torch.load('trained_model/kpcn_decomp_mask_2/diff1_e12.pt')
    ...
    # checkpointSpec1 = torch.load('trained_model/kpcn_decomp_mask_2/spec1_e12.pt')
    ...
    # checkpointDiff2 = torch.load('trained_model/kpcn_decomp_mask_2/diff2_e12.pt')
    ...
    # checkpointSpec2 = torch.load('trained_model/kpcn_decomp_mask_2/spec2_e12.pt')
    ...
    # init_epoch = checkpointDiff1['epoch']
    init_epoch = 0
    ...
    for epoch in range(init_epoch, epochs):
        ...
        for batch in tqdm(dataloader, leave=False, ncols=70):
            ...
            outputPre = outputDiff * (albedo + eps) + torch.exp(outputSpec) - 1.0
            target = batch['target_total'].to(device)
            target = crop_like(target, outputPre)
            error = torch.abs(target - outputPre)
            error = F.normalize(error)
            error = torch.ones_like(error) - error


            for k, v in batch.items():
                batch[k] = v.to(device)
            mask, batch1, batch2 = decompNet(batch)
            mask = crop_like(mask, error)
            loss_mask = criterion(error, mask) * 0.001
            # supervising mask
            ...
            writer.add_scalar('lossMask', loss_mask)
            ...
            writer.add_scalar('Train Mask loss 2', accuLossMask, epoch * len(dataloader) + i_batch)
