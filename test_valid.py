...
def denoise(models, dataloader, device, mode, save_dir, do_vis_feat, use_llpm_buf, debug=False):
    ...
    with torch.no_grad():
        ...
        diffuseNet, specularNet, preDiffuseNet, preSpecularNet = models['diffuse'].eval(), models['specular'].eval(), models['preDiffuse'].eval(), models['preSpecular'].eval()
...
def main():
    ...
    diffuseNet = KPCN(args.num_layers, args.input_channels, args.hidden_channels, args.kernel_size).to(args.device)
    specularNet = KPCN(args.num_layers, args.input_channels, args.hidden_channels, args.kernel_size).to(args.device)
    
    
    preDiffuseNet = KPCN(args.num_layers, 34, args.hidden_channels, args.kernel_size).to(args.device)
    preSpecularNet = KPCN(args.num_layers, 34, args.hidden_channels, args.kernel_size).to(args.device)
    ...
    checkpointDiff = torch.load('trained_model/kpcn/diff_e8.pt')
    checkpointSpec = torch.load('trained_model/kpcn/spec_e8.pt')
    preDiffuseNet.load_state_dict(checkpointDiff['model_state_dict'])
    preSpecularNet.load_state_dict(checkpointSpec['model_state_dict'])
    ...
    if args.use_llpm_buf:
        diffPathNet = PathNet(dataset.pnet_in_size).to(args.device)
        specPathNet = PathNet(dataset.pnet_in_size).to(args.device)
        models['path_diffuse'] = diffPathNet
        models['path_specular'] = specPathNet
    
    # test_model(diffuseNet, specularNet, args.device, args.data_dir, args.mode, args)
    _, _, totalL1, totalrelL2 = denoise(models, dataloader, args.device, args.mode, args.save_dir, args.do_vis_feat, args.use_llpm_buf)
    print('TEST L1 LOSS is {}'.format(totalL1))
    print('TEST L2 LOSS is {}'.format(totalrelL2))
